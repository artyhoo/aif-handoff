...
| `AGENT_MID_CYCLE_ADMISSION_ENABLED`                    | boolean | `true`                         | When a poll tick or wake event arrives during an active poll cycle, run a mid-cycle admission pass (auto-queue advance + parallel-project lanes) instead of waiting for the cycle's lanes to drain. Sequential projects always keep cycle-boundary semantics. Set to `false` to restore pure cycle-boundary admission                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
A poll cycle stays open until all of its project lanes drain. When a tick or wake event arrives during an active cycle and `AGENT_MID_CYCLE_ADMISSION_ENABLED=true` (default), the coordinator additionally runs a mid-cycle admission pass so eligible parallel-project work starts within one poll interval instead of waiting for the longest in-flight lane. See [Architecture → Mid-Cycle Admission](architecture.md#mid-cycle-admission).
