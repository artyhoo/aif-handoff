...
- [ ] If you touched the poll scheduler, verify periodic ticks and event-driven wakes share a single-flight coordinator loop; triggers received during an active cycle may request only one coalesced follow-up cycle, and (with `AGENT_MID_CYCLE_ADMISSION_ENABLED=true`, the default) also fire at most one single-flight mid-cycle admission pass — parallel-capable projects only, same semaphore/CAS claim path as cycle lanes.
