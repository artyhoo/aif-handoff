...
- [ ] Whenever a trigger is coalesced instead of starting an overlapping cycle, `AGENT_MID_CYCLE_ADMISSION_ENABLED=true` (the default) also fires at most one single-flight mid-cycle admission pass — parallel-capable projects only, same semaphore/CAS claim path as cycle lanes.
